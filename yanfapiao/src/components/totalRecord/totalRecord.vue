<template>
    <!-- <div class="check-home"> -->
        <!-- 全部记录 -->
        <div class="record-total" v-bind:style="{minHeight:screenHeight-30+'px'}">
            <search
            v-model="keyWord"
            position="fixed"
                ...
                <div class="box2">
                    <ul class="record-total-list">
                        <li class="record-total-item" v-for="item in invoiceList" :key="item.id" @click="toDetail(item)">
                            ...
                            <!-- <p class="item-footer">{{item.checkDate}}由{{item.inputuser}} 验证通过 <i class="success"></i></p> -->
...
import { getCheckNum,getFpInfo,getDictList } from '../../api/totalRecord'
export default {
  components: {
    Tab,
    TabItem,
  ...
  methods: {
    ...
    getFpInfo (type,pageNo) {
        ...
        getFpInfo({
            userId: this.userObj.USERID ? this.userObj.USERID : '',
            id: '',
            invoiceDataCode: this.invoiceDataCode,//	发票代码
            invoiceNumber: this.invoiceNumber,//	发票号码
            ...
            status: this.fpStateVal,//状态标识(取通用字典查询的value值)
            pageNo: pageNo,
            pageSize: 5,
        }).then((data) => {
    ...
    //获取编码对应汉字  type:1--发票类型  2--国税  3--状态标识
