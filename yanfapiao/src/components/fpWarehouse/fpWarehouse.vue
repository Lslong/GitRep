<template>
    <!-- <div class="check-home"> -->
        <!-- 全部记录 -->
        <div class="ware-house" v-bind:style="{minHeight:screenHeight-30+'px'}">
            <search
            v-model="keyWord"
            position="fixed"
                    ...
                    <div style="width:100%;margin:0 auto;">
                        <div class="box">
                            <ul class="select-list">
                                ...
                                <li>
                                    <h2 class="select-item-title" style="margin-top:0">发票日期</h2>
                                ...
                                <li>
                                    <h2 class="select-item-title">入账日期</h2>
                                    
                                    <group style="display:inline-block;vertical-align:middle">
                                        <datetime v-bind:class="{dataSelected: !voucherTime}" v-model="voucherTimeStart" @on-confirm="getSendStart" :end-date="nowData">
                                            {{voucherTimeStart}}
                                        </datetime>
                                    </group>
                                    <span style="vertical-align:middle;">--</span>
                                    <group style="display:inline-block;vertical-align:middle">
                                        <datetime v-bind:class="{dataSelected: !voucherTime}" v-model="voucherTimeEnd" @on-confirm="getSendEnd" :end-date="nowData">
                                            {{voucherTimeEnd}}
                                        </datetime>
                                    </group>  
                                ...
                                <li>
                                    <h2 class="select-item-title">凭证编号</h2>
                                    <group>
                                        <x-input v-model="voucherCode" type="number" placeholder="请输入凭证编号"></x-input>
                                    </group>
                                </li>
                            </ul>
...
import { getCheckNum,getFpRepInfo,getDictList } from '../../api/totalRecord'
export default {
  components: {
    Tab,
    TabItem,
  ...
  methods: {
    ...
    clearItems() {
      ...
      this.voucherTimeStart = "开始日期";
      this.voucherTimeEnd = "结束日期";
      this.voucherCode = "";
    },
    //确定筛选
    submitItems() {
    ...
    getFpInfo (type,pageNo) {
        ...
        getFpRepInfo({
            ...
            voucherCode: this.voucherCode,//	凭证编号
            beginBillingtime: this.sendTimeStart == "开始日期" ? "" : this.sendTimeStart,//	开票时间（启）（字符串格式yyyy-mm-dd）
            endBillingtime: this.sendTimeEnd == "结束日期" ? "" : this.sendTimeEnd,//	开票时间（止）（字符串格式yyyy-mm-dd）
            ...
            voucherDateBegin: this.voucherTimeStart == "开始日期" ? "" : this.voucherTimeStart,//	入账时间(启)（字符串格式yyyy-mm-dd）
            voucherDateEnd: this.voucherTimeEnd == "结束日期" ? "" : this.voucherTimeEnd,//	 入账时间(止)（字符串格式yyyy-mm-dd）
